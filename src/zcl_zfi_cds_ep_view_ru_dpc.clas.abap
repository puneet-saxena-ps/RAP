class ZCL_ZFI_CDS_EP_VIEW_RU_DPC definition
  public
...
protected section.
  ...
  methods ZFI_CDS_EP_VIEW__CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_ZFI_CDS_EP_VIEW_RU_MPC=>TS_ZFI_CDS_EP_VIEW_RULETYPE
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZFI_CDS_EP_VIEW__DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZFI_CDS_EP_VIEW__GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZFI_CDS_EP_VIEW__GET_ENTITYSET
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZFI_CDS_EP_VIEW_RU_MPC=>TT_ZFI_CDS_EP_VIEW_RULETYPE
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZFI_CDS_EP_VIEW__UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
CLASS ZCL_ZFI_CDS_EP_VIEW_RU_DPC IMPLEMENTATION.
...
*&* This class has been generated on 17.08.2023 05:32:36 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZFI_CDS_EP_VIEW_RU_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA zfi_cds_ep_view__create_entity TYPE zcl_zfi_cds_ep_view_ru_mpc=>ts_zfi_cds_ep_view_ruletype.
...
*-------------------------------------------------------------------------*
*             EntitySet -  ZFI_CDS_EP_VIEW_RULE
*-------------------------------------------------------------------------*
     WHEN 'ZFI_CDS_EP_VIEW_RULE'.
*     Call the entity set generated method
    zfi_cds_ep_view__create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
       	 ...
       	 IMPORTING er_entity          = zfi_cds_ep_view__create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = zfi_cds_ep_view__create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*-------------------------------------------------------------------------*
      when 'ZFI_CDS_EP_VIEW_RULE'.
*     Call the entity set generated method
     zfi_cds_ep_view__delete_entity(
          EXPORTING iv_entity_name     = iv_entity_name
                    iv_entity_set_name = iv_entity_set_name
                    iv_source_name     = iv_source_name
                    it_key_tab         = it_key_tab
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zfi_cds_ep_view__get_entity TYPE zcl_zfi_cds_ep_view_ru_mpc=>ts_zfi_cds_ep_view_ruletype.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          zfi_cds_ep_view__get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zfi_cds_ep_view__get_entity
                         es_response_context = es_response_context
          ).

        IF zfi_cds_ep_view__get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zfi_cds_ep_view__get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZFI_CDS_EP_VIEW_RU_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA zfi_cds_ep_view__get_entityset TYPE zcl_zfi_cds_ep_view_ru_mpc=>tt_zfi_cds_ep_view_ruletype.
 DATA lv_entityset_name TYPE string.
...
*-------------------------------------------------------------------------*
   WHEN 'ZFI_CDS_EP_VIEW_RULE'.
*     Call the entity set generated method
      zfi_cds_ep_view__get_entityset(
        EXPORTING
         iv_entity_name = iv_entity_name
         iv_entity_set_name = iv_entity_set_name
         iv_source_name = iv_source_name
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = zfi_cds_ep_view__get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = zfi_cds_ep_view__get_entityset
        CHANGING
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zfi_cds_ep_view__update_entity TYPE zcl_zfi_cds_ep_view_ru_mpc=>ts_zfi_cds_ep_view_ruletype.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*-------------------------------------------------------------------------*
      WHEN 'ZFI_CDS_EP_VIEW_RULE'.
*     Call the entity set generated method
          zfi_cds_ep_view__update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         iv_entity_set_name = iv_entity_set_name
                         iv_source_name     = iv_source_name
                         io_data_provider   = io_data_provider
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zfi_cds_ep_view__update_entity
          ).
       IF zfi_cds_ep_view__update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zfi_cds_ep_view__update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Unit test injection
  ...
  method IF_SADL_GW_DPC_UTIL~GET_DPC.
    TYPES ty_ZFI_CDS_EP_VIEW_RULE_1 TYPE zfi_cds_ep_view_rule ##NEEDED. " reference for where-used list

    DATA(lv_sadl_xml) =
               |<?xml version="1.0" encoding="utf-16"?>| &
               |<sadl:definition xmlns:sadl="http://sap.com/sap.nw.f.sadl" syntaxVersion="" >| &
               | <sadl:dataSource type="CDS" name="ZFI_CDS_EP_VIEW_RULE" binding="ZFI_CDS_EP_VIEW_RULE" />| &
               |<sadl:resultSet>| &
               |<sadl:structure name="ZFI_CDS_EP_VIEW_RULE" dataSource="ZFI_CDS_EP_VIEW_RULE" maxEditMode="RO" exposure="TRUE" >| &
    ...
    ro_dpc = cl_sadl_gw_dpc_factory=>create_for_sadl( iv_sadl_xml   = lv_sadl_xml
               iv_timestamp         = 20230817123234
               iv_uuid              = 'ZFI_CDS_EP_VIEW_RULE'
               io_context           = me->mo_context ).
  endmethod.


  method ZFI_CDS_EP_VIEW__CREATE_ENTITY.
  ...
  method ZFI_CDS_EP_VIEW__UPDATE_ENTITY.
